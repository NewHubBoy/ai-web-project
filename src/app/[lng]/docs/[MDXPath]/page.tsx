...
export default async function Page({ params }: { params: Params }) {
  ...
  return (
    <CommonContainer className="min-h-[calc(100vh-64px)] overflow-y-auto m-auto pb-16">
      <div className="max-w-[819px] px-4 m-auto text-[#272626]">
        <div className="my-16">
          ...
          <p className="md:text-[17px] text-sm tracking-wide leading-7 font-normal text-[#2b2b2b]">
            <span className="font-light">
              by{' '}
              <Link href={'/' + lng + '/author/' + metadata.author}>
                <span className="text-[#2f5aae]">{metadata.author}</span>
              </Link>
            </span>
          </p>
        </div>
        <MDXRemote
          ...
          components={{
            ...
            h2: ({ children }) => (
              <h2 style={{ fontSize: '1.65em', fontWeight: 600, lineHeight: 1.25 }} className="mb-8">
                {children}
              </h2>
            ),
        ...
        <PageFooter title={metadata?.title} lng={lng} />
      </div>
    </CommonContainer>
  );
}
